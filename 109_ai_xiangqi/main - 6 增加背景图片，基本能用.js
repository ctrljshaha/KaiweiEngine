...
// --- 7. 点击交互优化版 ---
scene.onPress((tx, ty) => {
    // 1. 修正坐标：tx, ty 是相对于屏幕的。
    // 我们需要计算点击点相对于“格点中心”的距离
    // 逻辑坐标 = (点击坐标 - 棋盘起点 - 棋子尺寸的一半) / 间距
    ...
    // 3. 点击有效性检查 (距离网格中心不能太远，例如 30 像素内)
    // 这样可以避免点击棋盘边缘或者格子正中间导致的误操作
    var dist = Math.sqrt(Math.pow(floatGx - gx, 2) + Math.pow(floatGy - gy, 2));
    if (dist > 0.8) return; // 0.8 是一个经验值，表示离中心点太远则无效
