...
function evaluateBoard(currentMap) {
    var score = 0;
    for (var y = 0; y < 10; y++) {
        for (var x = 0; x < 9; x++) {
            var piece = currentMap[y][x];
            if (piece) {
                var color = piece.indexOf("r_") === 0 ? "red" : "black";
                var type = piece.split("_")[1];
                var val = PIECE_VALUE[type] || 0;
                score += (color === "red" ? val : -val);
            }
        }
    }
    return score;
...
function aiMove() {
    ...
    for (var y = 0; y < 10; y++) {
        for (var x = 0; x < 9; x++) {
            if (map[y][x] && map[y][x].indexOf("b_") === 0) {
                ...
                for (var m of moves) {
                    allMoves.push({from: {x: x, y: y}, to: m});
                }
            }
        }
    }
    ...
    for (var i = 0; i < allMoves.length; i++) {
        var move = allMoves[i];
        var oldTarget = map[move.to.y][move.to.x];
        var oldSource = map[move.from.y][move.from.x];
        
        map[move.to.y][move.to.x] = oldSource;
        map[move.from.y][move.from.x] = null;
        var currentScore = evaluateBoard(map);
    ...
    if (bestMove) {
        history.push(JSON.parse(JSON.stringify(map))); // 记录 AI 走棋前的状态
