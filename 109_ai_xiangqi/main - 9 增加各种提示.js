...
function createMessageLabel() {
    ...
    messageLabel.setFont("font/st.ttf",15); // 标签汉字字库，字体大小
    scene.addNode(messageLabel); // 把标签增加到场景中
    
    } else {
        // 降级：使用 Sprite 绘制纯色背景+文本？简单起见，先不实现，但一般引擎都有 Label
        console.warn("Label not supported, message will not display.");
    }
}
...
function showMessage(msg, duration = 2000) {
    if (messageLabel) {
        ...
        messageTimeout = setTimeout(() => {
            if (messageLabel) messageLabel.setText("");//messageLabel.setVisible(false);
        }, duration);
    } else {
        // 后备（保留旧 alert 以便调试，但不弹窗，仅打印）
...
function alphaBeta(map, depth, alpha, beta, isMaximizing) {
    ...
    if (isMaximizing) {
        ...
        for (let move of moves) {
            ...
            let eval = alphaBeta(map, depth-1, alpha, beta, false);
        ...
        for (let move of moves) {
            ...
            map[move.to.y][move.to.x] = oldSource;
            map[move.from.y][move.from.x] = null;
            let eval = alphaBeta(map, depth-1, alpha, beta, true);
            map[move.from.y][move.from.x] = oldSource;
            map[move.to.y][move.to.x] = oldTarget;
            if (eval < minEval) {
                minEval = eval;
...
function aiMove() {
    ...
    for (let depth = 1; depth <= 4; depth++) {
        ...
        for (let move of moves) {
            ...
            map[move.to.y][move.to.x] = oldSource;
            map[move.from.y][move.from.x] = null;
            let eval = alphaBeta(map, depth-1, -Infinity, Infinity, false);
            map[move.from.y][move.from.x] = oldSource;
            map[move.to.y][move.to.x] = oldTarget;
...
scene.onPress((tx, ty) => {
    ...
    if (gameWinner !== null) {
        showMessage("游戏已经结束，请点击“重新开始”继续对弈。");  // REPLACED alert
        return;
    }
