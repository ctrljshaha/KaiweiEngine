...
scene.onPress((x, y) => {
    ...
    if (isGameOver) {
        resetGame(); // 先彻底重置
        menuSandia.setHide(false);
        menuCircle.setHide(false);
...
menuScene.upDate((time) => {
    ...
    // 原有旋转逻辑 例如：改成 5 会明显变快，改成 0.5 则会变得非常缓慢。
    menuSandia.setRotate(menuSandia.getRotate() - 1); // “3” 就是西瓜的旋转速度
    menuCircle.setRotate(menuCircle.getRotate() - 0.5); // “1” 就是外圈的旋转速度
});
