...
var resFlash = game.getResource().getTexture("img/flash.png");
...
menuScene.upDate((time) => {
    ...
    if (!menuBomb.isHide()) {
        flashTimer += time;
        if (flashTimer > 0.05) { // 每 0.05 秒吐出一张，造成“很多”的视觉感
            flashTimer = 0;
            
            var f = new Sprite();
            f.setTexture(resFlash); // 修改点 3：确保吐出的是 flash.png 而不是白色菱形
            var s = 30 + Math.random() * 20; // 随机初始大小
            f.setSize(s, s);
            
            var bPos = menuBomb.getPosition();
            // 修改点 1：在地雷图片的左上角吐出 (原来是 +50, +10，改为 -10 左右)
            f.setPosition(bPos.x - 10, bPos.y - 10); 
            
            // 随机发散方向（主要向上和向左）
            f.vx = -2 - Math.random() * 4; // 改为向左运动
            f.vy = -3 - Math.random() * 5; // 保持向上运动
            ...
            f.upDate((time) => {
                ...
                if (life <= 0) {
                    f.setHide(true);
                    // 引擎会自动清理隐藏的节点，这里设为隐藏即可
                } else {
                    var pos = f.getPosition();
                    // 简单的物理运动
                    f.vy += GRAVITY * 0.5; // 受轻微重力影响
                    f.setPosition(pos.x + f.vx, pos.y + f.vy);
                    
                    // 图片由大极小
                    var scale = life / 5.0; 
                    f.setSize(initS * scale, initS * scale);
                    f.setColor(255, 255, 255, scale); // 透明度也随之衰减
                }
            });
            menuScene.addNode(f, 999); // 确保在最上层
        }
    }
});
