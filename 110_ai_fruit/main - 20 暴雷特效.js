...
function resetGame() {
    ...
    for (var i = 0; i < 3; i++) {
        ...
        lifeIcons[i].setSize(lifeIcons[i].originSize, lifeIcons[i].originSize); // 恢复大小
    }
    if (maskNode) maskNode.setHide(true);
    if (gameOverNode) gameOverNode.setHide(true);
    if (whiteFlashNode) whiteFlashNode.setHide(true); // 隐藏白屏
...
// --- 修改点：地雷爆炸特效与全屏白光 ---
var whiteFlashNode; // 全屏白光节点
function triggerBomb(bombNode) {
    if (isGameOver) return;
    isGameOver = true; // 立即锁定状态
    ...
    // 2. 产生白色条纹射出效果（花朵绽放式的白条）
    for (var i = 0; i < 8; i++) {
        var ray = new Node();
        ray.setColor(255, 255, 255, 1);
        var angle = i * 45; // 均匀分布8个方向
        ray.setRotate(angle);
        ray.setPosition(pos.x, pos.y);
        ray.setSize(10, 2); // 初始很短
        ...
        (function(node, startAngle) {
            ...
            node.upDate((dt) => {
                rayLife += dt;
                var currentLen = rayLife * 2000;
                var currentAlpha = 1.0 - (rayLife / 0.5);
                if (currentAlpha <= 0) {
                    node.setHide(true);
                    scene.removeNode(node);
                } else {
                    node.setSize(currentLen, 15 * currentAlpha);
                    node.setColor(255, 255, 255, currentAlpha);
                    node.setRotate(startAngle + rayLife * 100); // 带一点旋转旋转
                }
            });
        })(ray, angle);
    }
    ...
    var flashAlpha = 1.0;
    var waitTimer = 0;
    whiteFlashNode.upDate((dt) => {
        waitTimer += dt;
        if (waitTimer > 0.1) { // 稍微白一下就开始消退
            flashAlpha -= 0.02;
            if (flashAlpha <= 0) {
                flashAlpha = 0;
                whiteFlashNode.setHide(true);
                // 4. 白光消退完，回归显示 GameOver 界面
                triggerGameOverUI();
            }
    ...
    scene.upDate((time) => {
        shakeTimer += time;
        if(shakeTimer < 0.6) {
            scene.setPosition(Math.random()*30-15, Math.random()*30-15);
        } else { scene.setPosition(0,0); }
    });
}

var maskNode, gameOverNode; 
// 修改点：将UI显示拆分为独立函数，以便地雷特效后调用
...
function triggerGameOver() {
    if (isGameOver) return;
    isGameOver = true;
    triggerGameOverUI(); // 直接进入UI逻辑（非地雷死亡情况）
}
...
// 修改点：定义一个 Flash 纹理资源，使用用户提供的图片
var resFlash = game.getResource().getTexture("img/circle.png");
var flashTimer = 0; // 用于控制 Flash 图片喷出的计时器
...
menuScene.upDate((time) => {
    ...
    // 修改点 1 和 2：修改喷出位置为左上角，并处理地雷隐藏后的状态
    if (!menuBomb.isHide()) {
        flashTimer += time;
        if (flashTimer > 0.15) { // 每 0.15 秒吐出一张
            flashTimer = 0;
            
            var f = new Sprite();
            f.setTexture(resFlash); 
            // 2. 修改：吐出的范围缩小，现在吐的太大了
            var s = 5 + Math.random() * 8; // 随机初始大小进一步缩小
            f.setSize(s, s);
            ...
            // 2. 修改：在小范围就消失，生命周期减短
            var life = 0.8; // 从 5.0 减少到 0.8 秒，实现“小范围消失”
            var initS = s;

            // 1. 修改：增强饱和度的随机颜色逻辑
            // 原理：随机选一个通道为 255 (极亮)，一个通道为 0 (极暗)，最后一个通道随机
